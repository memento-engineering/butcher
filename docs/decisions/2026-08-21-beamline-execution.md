...
- Recompilation is not the cost ([0010](2026-08-14-mutant-schemata.md)); loading a
...
- A beamline is a worker process that loads the schemata build once
  ([0010](2026-08-14-mutant-schemata.md)) and runs exposure after exposure against
  it.
...
- A beamline is rebuilt whenever it cannot be trusted: after a hang, after a
  load-time mutant ([0010](2026-08-14-mutant-schemata.md)), and after a fixed
  number of exposures.
- `package:test`'s runner API is the contract; its reporter events stay the
  evidence behind every verdict ([0006](2026-08-14-outcome-taxonomy.md)).
...
- The calibration run records a source report per test, which is what
  per-test routing needs ([0011](2026-08-14-per-test-coverage-routing.md)).
...
- Determinism narrows: outcomes, mutant IDs, and report order stay
  ...
  not ([0007](2026-08-14-deterministic-execution.md)).
...
- A hosted suite still spawns processes of its own; the kill discipline in
  [0006](2026-08-14-outcome-taxonomy.md) applies to a beamline as it does to a
  suite.
