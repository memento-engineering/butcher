...
- Crashed runs leave sandboxes and logs under the resolved butcher temp root.
...
- Concurrent cleanup must not remove another active run's sandbox.

## Decision

- Startup cleanup is the first run stage.
- The CLI acquires an exclusive `<butcher temp root>/.lock` before cleanup.
...
- After acquiring the lock, startup cleanup removes:
  ...
  - every top-level `sandbox_*` directory;
...
- No sandbox or evidence cleanup runs at exit.

## Consequences

- Every run starts without abandoned sandboxes or logs.
