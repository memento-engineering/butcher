...
- One tool logger (`ButcherLogger`) per run is created by the CLI and shared with
  the engine for start, report, exit, and orchestration events.
- Each worker appends mutant-run events to its sandbox log through the
...
- Tool log at `<system temp>/butcher/butcher.log`. Startup cleanup removes the previous
...
|---|---|
| `starting butcher {ToolVersion} on {ProjectRoot} with {Jobs} jobs` | Dart version, OS, argv |
| `classified {MutantId} as {Outcome} ({Done}/{Total})` | file, offset, operator, replacement, exit code, timed out, duration |
| `run complete: MSI {Msi}% over {MutantCount} mutants, exit {ExitCode}` | outcome counts, covered MSI, baseline, deadline, duration, report path |
...
- Mutated-run suite output is kept under `<system temp>/butcher/runs/`.
...
- Each worker writes one `<sandbox-name>.log` file. The random sandbox
...
- Every executed mutant appends one wide event carrying its mutation context,
  suite output, sandbox name, and shared `RunId`.
...
- Mutants with no coverage do not execute and produce no sandbox event.
