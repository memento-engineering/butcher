...
import 'package:butcher/src/engine/outcome_classifier.dart';
...
''',
  ...
  test(
    'yields an empty process snapshot when the image has no lister',
    () async {
  ...
  test('snapshots this process among the running ones', () async {
    final listed = DartTestRunner.descendantPids(
      await DartTestRunner.processSnapshot(),
      {pid},
    );
    ...
    expect(
      listed,
      isNotEmpty,
      reason: 'the suite runs its tests in child processes of this one',
    );
  });
...
''';
  ...
  test('sweeps what a killed process spawned before it died', () {
    // 200 is gone from the snapshot, but still names 300's parent.
    expect(DartTestRunner.descendantPids('300 200\n', {100, 200}), [300]);
  });
}
